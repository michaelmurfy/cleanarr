...
def _unwrap_user(value: Any) -> Any:
    """Pull a username out of nested media-server/Seerr user objects (or their str() form)."""
...
class UserDirectory:
    """Merge Seerr / Tautulli / Jellystat identities, preferring the media-server account username."""

    def __init__(self) -> None:
        self.people: list[dict[str, Any]] = []

    def ingest_seerr(self, user: dict[str, Any]) -> dict[str, Any]:
        """Seerr logins can be Plex or Jellyfin; either is the account username Tautulli/Jellystat report."""
        return self.add(
            account=(
                user.get("plexUsername")
                or user.get("plex_username")
                or user.get("jellyfinUsername")
                or user.get("jellyfin_username")
                or ""
            ),
            ...
            extras=[
                ...
                user.get("plexUsername") or user.get("plex_username") or "",
                user.get("jellyfinUsername") or user.get("jellyfin_username") or "",
    ...
    def ingest_tautulli(self, user: dict[str, Any]) -> dict[str, Any]:
        return self.add(
            account=user.get("username") or user.get("user") or "",
    ...
    def add(
        ...
        *,
        ...
        email = _clean(email)
        aliases = {_key(item) for item in [account, display, email, *(extras or [])] if _clean(item)}
        if not aliases:
            return {"canonical": "", "display": "", "account": "", "email": "", "aliases": []}

        person = None
        account_key = _key(account)
        email_key = _key(email)
        for candidate in self.people:
            if account_key and _key(candidate.get("account")) == account_key:
                person = candidate
                break
        ...
        if person is None:
            person = {
                "account": account,
                "display": display or account or email,
                "email": email,
                "aliases": set(),
                "seerr_id": seerr_id,
        ...
        if account and (not person.get("account") or _key(person.get("account")) == _key(person.get("display"))):
            person["account"] = account
        if display and (not person.get("display") or person["display"] == person.get("account")):
    ...
    def resolve(self, raw: Any) -> dict[str, str]:
        if isinstance(raw, dict):
            person = self.add(
                account=(
                    raw.get("plexUsername")
                    or raw.get("plex_username")
                    or raw.get("jellyfinUsername")
                    or raw.get("jellyfin_username")
                    or raw.get("user")
                    or raw.get("username")
                    or raw.get("UserName")
                    or raw.get("Name")
            ...
            if needle:
                for candidate in self.people:
                    if needle in candidate["aliases"] or needle == _key(candidate.get("account")):
                        person = candidate
                        break
            if person is None and text:
                # Stringified media-server/Seerr user objects should land as account usernames.
                as_account = isinstance(raw, str) and raw.strip().startswith("{") and text != raw.strip()
                person = self.add(account=text if as_account else "", display=text, extras=[text])
            if person is None:
                return {"canonical": "", "display": "", "account": "", "email": ""}
        display = person.get("display") or person.get("account") or ""
        account = person.get("account") or ""
        canonical = account or display
        return {
            "canonical": canonical,
    ...
    def snapshot(self) -> list[dict[str, Any]]:
        ...
        for person in self.people:
            account = person.get("account") or ""
            display = person.get("display") or account
            canonical = account or display
            if not canonical:
                continue
            rows.append(
