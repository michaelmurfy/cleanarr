...
**Matching.** `CatalogIndex` matches tmdb → tvdb → imdb, then unique normalized primary title with exact year or ±1 (yearless only when the query has no year). Alternate / original titles only match with an exact year. Each linked row stores `seerr_match_via` and `seerr_title`. Title-guessed links (`title` / `title_alt`) are listed under Unmatched → Check matches, never on the Library page; the user either keeps or unlinks each one (`match_ignored.action`), an unlinked pair stays detached across syncs, and pending checks count toward `stats.actionable`. Unmatched is Seerr ↔ *arr only. Ignore titles named `Unknown`. Skip Seerr blocklisted (status 6) and deleted (status 7) when deciding stale. `mediaAddedAt` alone is not proof Seerr still has the title. Seerr is keyed on TMDB, so a row with no TMDB id can never be added there.
