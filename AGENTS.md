...
**Matching.** `CatalogIndex` matches tmdb → tvdb → imdb, then unique normalized primary title with exact year or ±1 (yearless only when the query has no year). Alternate / original titles only match with an exact year. Wrong Seerr↔*arr pairs can be unlinked from the library card (`match_ignored`) and stay rejected across syncs; each linked row stores `seerr_match_via`. Unmatched is Seerr ↔ *arr only. Ignore titles named `Unknown`. Skip Seerr blocklisted (status 6) and deleted (status 7) when deciding stale. `mediaAddedAt` alone is not proof Seerr still has the title. Seerr is keyed on TMDB, so a row with no TMDB id can never be added there.
